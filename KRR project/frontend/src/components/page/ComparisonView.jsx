...
const ComparisonView = () => {
  ...
  const fetchComparison = async () => {
    ...
    setError(null);
    try {
      const libRes = await fetch(`${API_URL}/api/library`);
      if (!libRes.ok) throw new Error('Could not reach backend.');
      ...
      const compareRes = await fetch(`${API_URL}/api/compare`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ paper_ids: library.map((p) => p.id) }),
      });

      if (!compareRes.ok) {
        const err = await compareRes.json().catch(() => ({}));
        throw new Error(err.detail || `Server error: ${compareRes.status}`);
      }
